...
IMAGE_NAME=toolsbeta-harbor.wmcloud.org/toolforge/builds-api:dev
...
rollout: check_requirements
	bash -c "if kubectl get namespace builds-api >/dev/null 2>&1; then kubectl rollout restart -n builds-api deployment builds-api; else :; fi"
