...
The core of the app is generated by go-swagger, from the swagger yaml file, so
every time you modify the swagger yaml definiton, you have to regenerate the
code, to do so just:
...
Then you can manually modify the code under `cmd/builds-api` to match the new
...
Then you might have to tweak `cmd/secrets-api/main.go` to add any new handlers,
remove old ones or change any if needed.

## On the client side

This one is a bit more tricky, as more changes were needed there. NOTE: It's
easier if you don't have any local changes to any other go files, commit/stash
them if you do.
