...
func get_pipelineRunParam(pipelineRun v1beta1.PipelineRun, name string) string {
...
func getBuild(run v1beta1.PipelineRun) *gen.Build {
	...
	sourceurl := get_pipelineRunParam(run, "SOURCE_URL")
	ref := get_pipelineRunParam(run, "SOURCE_REFERENCE")
	destinationimage := get_pipelineRunParam(run, "APP_IMAGE")
...
func Get(
	...
	if len(pipelineRuns) == 0 {
		message := fmt.Sprintf("Build with id %s not found. Nothing defined yet.", id)
		return http.StatusNotFound, gen.NotFound{Message: &message}, nil
	}

	if len(pipelineRuns) > 1 {
		log.Warnf(
			"Found multiple (%d) pipelineRun objects with id %s on namespace %s", len(pipelineRuns), id, api.Config.BuildNamespace,
		)
	}
