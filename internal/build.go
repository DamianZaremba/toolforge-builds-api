...
func Logs(api *BuildsApi, buildId string, namespace string, toolName string) (int, interface{}, error) {
	...
	pipelineRuns, err := getPipelineRuns(&api.Clients, namespace, metav1.ListOptions{FieldSelector: fmt.Sprintf("metadata.name=%s", buildId)})
	if err != nil {
		log.Warnf(
	...
	logs, err := getPipelineRunLogs(&pipelineRun, &api.Clients, namespace)
	if err != nil {
...
func Start(
	...
	cleanup_err := cleanupOldPipelineRuns(&api.Clients, namespace, toolName, api.Config.OkToKeep, api.Config.FailedToKeep)
	...
	if err != nil {
		message := fmt.Sprintf("Got error when creating a new pipelinerun on namespace %s: %s", namespace, err)
		log.Warn(message)
		return http.StatusInternalServerError, gen.InternalError{Message: &message}, nil
	}
...
func Delete(
	...
	err := api.Clients.Tekton.TektonV1beta1().PipelineRuns(namespace).Delete(
		context.TODO(),
		buildId,
		metav1.DeleteOptions{},
	)
