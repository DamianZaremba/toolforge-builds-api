...
func TestCancelReturnsCancelledBuild(t *testing.T) {
	...
	gottenBuild := response.(gen.Build)
	if *gottenBuild.BuildId != buildId {
		t.Fatalf("Got unexpected build id, expected '%s', got '%s'", buildId, *gottenBuild.BuildId)
	}
	if *gottenBuild.Status != "BUILD_CANCELLED" {
		t.Fatalf("Got unexpected build status, expected 'cancelled', got '%s'", *gottenBuild.Status)
	}
}
